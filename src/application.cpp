...
Application::Application(int window_width, int window_height, SDL_Window* window)
{
	...
	// Create camera
	camera = new Camera();
	camera->lookAt(Vector3(-5.f, 1.5f, 11.f), Vector3(0.f, 0.0f, 0.f), Vector3(0.f, 1.f, 0.f));
	camera->setPerspective(45.f,window_width/(float)window_height,0.1f,10000.f); //set the projection, we want to be perspective

	//the first node is always the skybox node
	skybox_node = new SkyboxNode("Skybox", "", true); //creating skybox node
	node_list.push_back(skybox_node);

	//helmet
	SceneNode * helmet = new SceneNode("Helmet");
	helmet->mesh = Mesh::Get("data/models/helmet/helmet.obj");
	//textures
	const char* texture_names[] = { {"data/models/helmet/albedo.png"},
									{"data/models/helmet/roughness.png"},
									{"data/models/helmet/metalness.png"},
									{"data/textures/brdfLUT.png"} ,
									{"data/models/helmet/normal.png"},
									{"data/models/helmet/ao.png"},
									{"data/models/lantern/opacity.png"},
									{"data/models/helmet/emissive.png"} };

	helmet->material = new PBRmaterial(texture_names, false, true);
	node_list.push_back(helmet);

	//lantern
	SceneNode * lantern = new SceneNode("Lantern");
	lantern->mesh = Mesh::Get("data/models/lantern/lantern.obj");

	const char* texture_names2[] = { {"data/models/lantern/albedo.png"},
									{"data/models/lantern/roughness.png"},
									{"data/models/lantern/metalness.png"},
									{"data/textures/brdfLUT.png"} ,
									{"data/models/lantern/normal.png"},
									{"data/models/lantern/ao.png"},
									{"data/models/lantern/opacity.png"},
									{"data/models/helmet/emissive.png"}};

	lantern->material = new PBRmaterial(texture_names2, true, false);
	lantern->model.scale(0.05, 0.05, 0.05);
	lantern->model.translate(20, -40, -80);
